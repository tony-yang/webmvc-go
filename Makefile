...
test: build-container
	@docker run --rm webmvc-go bash -c 'cd dev/src/webmvc && for p in . ./controllers; do echo ""; echo "######## Test $$p ########"; go test $$p -v -covermode=count -coverprofile=webmvc_cover.out; go tool cover -func=webmvc_cover.out; echo "######## END Test $$p ########"; echo ""; done'
